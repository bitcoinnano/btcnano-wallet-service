...
[![NPM Package](https://img.shields.io/npm/v/btcnano-wallet-service.svg?style=flat-square)](https://www.npmjs.org/package/btcnano-wallet-service)
[![Build Status](https://img.shields.io/travis/bitcoinnano/btcnano-wallet-service.svg?branch=master&style=flat-square)](https://travis-ci.org/bitcoinnano/btcnano-wallet-service)
[![Coverage Status](https://coveralls.io/repos/bitcoinnano/btcnano-wallet-service/badge.svg?branch=master)](https://coveralls.io/r/bitcoinnan/btcnano-wallet-service?branch=master)

A Multisig HD Bitcore Wallet Service.

# Description

Btcnano Wallet Service facilitates multisig HD wallets creation and operation through a (hopefully) simple and intuitive REST API.
...
More about BWS at https://blog.bitpay.com/announcing-the-btcnano-wallet-suite/

# Getting Started
```
 git clone https://github.com/bitcoinnano/btcnano-wallet-service.git
 cd btcnano-wallet-service && npm start
```
 ...
 * A copayer could join the wallet more than once, and there is no mechanism to prevent this. See [wallet](https://github.com/bitcoinnano/btcnano-wallet)'s confirm command, for a method for confirming copayers.
...
Returns:
 * Wallet object. (see [fields on the source code](https://github.com/bitcoinnano/btcnano-wallet-service/blob/master/lib/model/wallet.js)).
...
Returns:
 * List of pending TX Proposals. (see [fields on the source code](https://github.com/bitcoinnano/btcnano-wallet-service/blob/master/lib/model/txproposal.js))

`/v1/addresses/`: Get Wallet's main addresses (does not include change addresses)

Returns:
 * List of Addresses object: (https://github.com/bitcoinnano/btcnano-wallet-service/blob/master/lib/model/address.js)).  This call is mainly provided so the client check this addresses for incoming transactions (using a service like [Insight](https://insight.is)
...
Returns:
 * TX Proposal object. (see [fields on the source code](https://github.com/bitcoinnano/btcnano-wallet-service/blob/master/lib/model/txproposal.js)). `.id` is probably needed in this case.


`/v1/addresses/`: Request a new main address from wallet

Returns:
 * Address object: (https://github.com/bitcoinnano/btcnano-wallet-service/blob/master/lib/model/address.js)). Note that `path` is returned so client can derive the address independently and check server's response.
...
Returns:
 * TX Proposal object. (see [fields on the source code](https://github.com/bitcoinnano/btcnano-wallet-service/blob/master/lib/model/txproposal.js)). `.status` is probably needed in this case.
